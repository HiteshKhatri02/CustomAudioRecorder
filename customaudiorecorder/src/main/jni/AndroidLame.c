...
JNIEXPORT void JNICALL Java_com_recorder_customaudiorecorder_AndroidLame_initializeDefault(
        JNIEnv *env, jclass cls) {

    glf = initializeDefault(env);
}

JNIEXPORT void JNICALL Java_com_recorder_customaudiorecorder_AndroidLame_initialize(
...
JNIEXPORT jint JNICALL Java_com_recorder_customaudiorecorder_AndroidLame_lameEncode(
...
JNIEXPORT jint JNICALL Java_com_recorder_customaudiorecorder_AndroidLame_encodeBufferInterleaved(
        JNIEnv *env, jclass cls, jshortArray pcm,
        jint samples, jbyteArray mp3buf) {
    return encodeBufferInterleaved(env, glf, pcm, samples, mp3buf);
}

JNIEXPORT jint JNICALL Java_com_recorder_customaudiorecorder_AndroidLame_lameFlush(
        JNIEnv *env, jclass cls, jbyteArray mp3buf) {
    return flush(env, glf, mp3buf);
}

JNIEXPORT void JNICALL Java_com_recorder_customaudiorecorder_AndroidLame_lameClose(
        JNIEnv *env, jclass cls) {
    close(glf);
}
